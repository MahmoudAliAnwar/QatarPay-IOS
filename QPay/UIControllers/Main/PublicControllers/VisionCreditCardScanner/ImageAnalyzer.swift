...
protocol ImageAnalyzerProtocol: AnyObject {
    func didFinishAnalyzation(with result: Result<VisionCreditCard, CreditCardScannerError>)
}
...
final class ImageAnalyzer {
    ...
    private var selectedCard = VisionCreditCard()
    ...
    func analyze(image: CGImage) {
        ...
        self.selectedCard.image = UIImage(cgImage: image)
    ...
    lazy var requestHandler: ((VNRequest, Error?) -> Void)? = { [weak self] request, _ in
        ...
        var creditCard = VisionCreditCard(number: nil, name: nil, expireDate: nil)
