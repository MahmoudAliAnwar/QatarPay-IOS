...
protocol CheckBoxDesign: AnyObject {
    var style: CheckBox.Style { get }
    var borderStyle: CheckBox.BorderStyle { get }
    
    var borderWidth: CGFloat { get }
    var checkedborderColor: UIColor { get }
    var borderColor: UIColor { get }
    var backgroundColor: UIColor { get }
    var checkmarkColor: UIColor { get }
    
    var startCheckedStatus: Bool { get }
    
//    var borderColor: UIColor { get }
//    var unCheckedborderColor: UIColor { get }
}

protocol CheckBoxDelegate: AnyObject {
    
    var checkBoxDesign: CheckBoxDesign { get }
    
    func didChecked(_ isChecked: Bool)
    func didTappedBox(_ box: CheckBox)
}
...
open class CheckBox: UIControl {
    ...
    weak var delegate: CheckBoxDelegate! {
        willSet {
            self.borderStyle = newValue.checkBoxDesign.borderStyle
            self.style = newValue.checkBoxDesign.style
            
            self.checkmarkColor = newValue.checkBoxDesign.checkmarkColor
            self.checkedBorderColor = newValue.checkBoxDesign.checkedborderColor
            self.backgroundColor = newValue.checkBoxDesign.backgroundColor
            self.tintColor = newValue.checkBoxDesign.borderColor
            self.borderWidth = newValue.checkBoxDesign.borderWidth
            self.isChecked = newValue.checkBoxDesign.startCheckedStatus
        }
    }
    ...
    var isChecked: Bool = false {
        ...
        willSet {
            if self.delegate != nil {
                self.delegate.didChecked(newValue)
                self.delegate.didTappedBox(self)
            }
        }
    }
